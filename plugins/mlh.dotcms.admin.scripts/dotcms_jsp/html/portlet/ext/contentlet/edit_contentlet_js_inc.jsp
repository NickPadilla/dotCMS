	...
	function saveContentCallback(data){
		...
		// if we have a referer and the contentlet comes back checked in
	 	if(data["referer"] != null && data["referer"] != '' && !data["contentletLocked"] ) {
			var andOrQues = data["referer"].indexOf("?") > -1 ? "&" : "?";
			<%if(structure.getStructureType()==Structure.STRUCTURE_TYPE_FORM){%>
				//self.location = data["referer"] + andOrQues + "structure_id=<%=structure.getInode()%>&content_inode=" + data["contentletInode"];
			<%}else{%>
				self.location = data["referer"] + andOrQues + "content_inode=" + data["contentletInode"];
			<%}%>
			return;
		}
	 	resetHasChanged();
...
<%--
MLH add on - give the ability to load scripts dynamically from other scripts.
 --%>
<script type="text/javascript" src="/html/js/dotcms/mlh-utils/js/load_script.js"></script>