...
//								String quotes = fieldValue.contains(" ") ? "\"" : "";
	...
	public Map<String,Object> saveContent(List<String> formData, boolean isAutoSave,boolean isCheckin, boolean publish) throws LanguageException, PortalException, SystemException {
		...
		String MLH_REFERER_OVERRIDE = "mlhRefererOverride";
		...
		if(!isAutoSave
			...
			if (UtilMethods.isSet(language) && referer.indexOf("language") > -1) {
				...
				String langParam = com.dotmarketing.util.WebKeys.HTMLPAGE_LANGUAGE + "=" + language;
				referer = referer.replaceAll("language=([0-9])*", langParam);
				int lastForwardSlashIndex = referer.lastIndexOf("/");
				if (lastForwardSlashIndex == (referer.length()-1)) {
					referer = referer.substring(0, lastForwardSlashIndex);
				}
				Logger.debug(this, "Referer after being replaced=" + referer);
			}
			if (referer.indexOf(MLH_REFERER_OVERRIDE) > -1) {
				int startIndex = referer.indexOf("=", referer.indexOf(MLH_REFERER_OVERRIDE)) + 1;
				int nextParam = referer.indexOf("&", startIndex);
				int endIndex = nextParam > -1 ? nextParam : referer.length();
				referer = referer.substring(startIndex, endIndex) + "?mlhOverride=meh";
			}
		}
		if(!isAutoSave){
